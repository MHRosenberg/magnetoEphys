...
LOAD_MOST_RECENT_REC = 1 # use 1 to load the most recent and 0 if you wish to specify the path
...
if LOAD_MOST_RECENT_REC ==1:
	...
	latest_subdir = max(glob.glob(os.path.join(DIR_OF_RECORDINGS, '*\\')), key=os.path.getmtime)
	pathToFile = latest_subdir + '\\100_CH1.continuous'
	RAW_DATA_PATH = latest_subdir
elif LOAD_MOST_RECENT_REC == 0:
	...
	print('ERROR: specify 0 or 1 for LOAD_MOST_RECENT_REC param')
